...
double BLI_drand()
{
	return (((double)(rand()) / RAND_MAX) - 0.0) * 1;
}
...
void BLI_initjit(float *jitarr, int num)
{
	...
	jit2= malloc(12 + 2*sizeof(float)*num);
...
int main(int argc, char** argv)
{
	...
	float jit[JITSIZE][2];
	printf("/* jitter table\n");
	printf("   blatantly stolen from a Blender algorithm */\n\n");
	printf("#define _J_MAX_SUPERSAMPLING %i\n\n", JITSIZE);
	printf("float _j_jitter[%i][%i][2] = {\n", JITSIZE - cursize + 1, JITSIZE);
	while (cursize <= JITSIZE)
	{
		printf("\n{ /* jitter for %i samples */\n", cursize);
		BLI_initjit(jit[0], cursize);
		unsigned int i;
		for (i = 0; i < cursize; ++i)
			printf("{%f, %f},\n", jit[i][0], jit[i][1]);
		for (i = cursize; i < JITSIZE; ++i)
			printf("{0.0, 0.0},\n");
		printf("},\n");
		cursize++;
	}
